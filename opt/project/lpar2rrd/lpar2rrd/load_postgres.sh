...
for MYDB in $POSTGRES_LIST
  ...
  PROC=`ps -ef | egrep 'postgres-db2conf.pl *'$MYDB' *$' | grep -v grep | wc -l|sed 's/ //g'`
  if [ $PROC -lt 1 ]; then
    $PERL -w $BINDIR/postgres-db2conf.pl $MYDB 2>>$ERRLOG &
    echo "postgres-db2conf.pl : status $status, "`date`

  else
    date_act=`date "+%Y-%m-%d_%H:%M"`
    echo "$0: $date_act : Command postgres-db2conf.pl is already running for $MYDB, cannot start the next one"
    echo "$0: $date_act : Command postgres-db2conf.pl is already running for $MYDB, cannot start the next one"  2>>$ERRLOG
  fi
  ...
  if [ ! "$XORMON"x = "x" ]; then
    if [ "$XORMON" != "0" ]; then
      echo "postgres-json2db.pl             : db, "`date`
