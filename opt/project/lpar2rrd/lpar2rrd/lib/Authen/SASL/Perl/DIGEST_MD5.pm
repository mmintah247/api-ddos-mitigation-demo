...
my @ourciphers = (
  { name  => 'rc4',
    ...
    fixup => sub {
      ...
      *Crypt::RC4::encrypt   = *Crypt::RC4::decrypt = sub { goto &Crypt::RC4::RC4; };
  ...
  { name => '3des',
    ...
    key  => sub {
      pack(
        'B8' x 16,
        map   { $_ . '0' }
          map { unpack( 'a7' x 16, $_ ); } unpack( 'B*', substr( $_[0], 0, 14 ) )
      );
    },
  ...
  { name => 'des',
    ...
    key  => sub {
      pack(
        'B8' x 8,
        map   { $_ . '0' }
          map { unpack( 'a7' x 8, $_ ); } unpack( 'B*', substr( $_[0], 0, 7 ) )
      );
    },
    iv => sub { substr( $_[0], -8, 8 ) },
  },
...
sub client_step {    # $self, $server_sasl_credentials
  ...
  my ( $response, $rspauth ) = $self->_compute_digests_and_set_keys( $password, \%response );
...
sub _compute_digests_and_set_keys {
  ...
  my $response = md5_hex( join( ":", $hdA1, @$params{qw(nonce nc cnonce qop)}, md5_hex($A2) ) );
  ...
  my $rspauth = md5_hex( join( ":", $hdA1, @$params{qw(nonce nc cnonce qop)}, md5_hex($A2) ) );
...
sub server_step {
  ...
  my $response_check = sub {
    ...
    my ( $expected, $rspauth ) = $self->_compute_digests_and_set_keys( $password, $self->{client_params} );
...
sub _parse_challenge {
  ...
  while (
    $$challenge_ref =~ s/^(?:\s*,)*\s*            # remaining or crap
             ([\w-]+)                 # key, eg: qop
             =
...
sub _select_cipher {
  ...
  my @a = map {
    my $c = $_;
    ( grep { $c->{name} eq $_ } @$ciphers and $c->{ssf} >= $minssf and $c->{ssf} <= $maxssf )
      ? $_
      : ()
  } @ourciphers;
