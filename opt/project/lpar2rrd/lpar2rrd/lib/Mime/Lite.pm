...
sub build {
  ...
  while (@paramz) {
    ...
    elsif ( $KnownField{$lc_tag}
      or $lc_tag =~ m{^(content|resent|x)-.} )
    {
      $field = $lc_tag;
    }
    else {                              ### not a field:
...
sub fields {
  ...
  foreach $tag ( sort keys %{ $self->{Attrs} } ) {
    ...
    if ( my $subs = $sub_attrs->{$tag} ) {
      $value .= '; ' . join( '; ', map {qq{$_="$subs->{$_}"}} sort keys %$subs );
    }
  ...
  if (@order) {                                  ### either?
    ...
    my @ranked = map { [ ( $_ + 1000 * ( $rank{ lc( $fields[$_][0] ) } || ( 2 + $#order ) ) ), $fields[$_] ] } ( 0 .. $#fields );
