    ...
    while ( $ypos >= $bottom_border + $line_space ) {
        ...
        while ( @paragraph ) {
            if ( !@line ) {
                ...
                ###########################################
                # KZ fix
                #my $num    = $text_object->advancewidth( $paragraph[0] );
                #warn "#### num        : $num";
                #warn "#### num        : " . sprintf "%.1000g", $num;
                # -->
                # #### num        : 58.38 at /home/stor2rrd/stor2rrd/lib/PDF/Table.pm line 1725.
                # #### num        : 58.380000000000003 at /home/stor2rrd/stor2rrd/lib/PDF/Table.pm line 1726.
                # !!! Error !!! 58.38 <= 58.38 at /home/stor2rrd/stor2rrd/lib/PDF/Table.pm line 1736.

                #if ( $text_object->advancewidth( $paragraph[0] ) +
                #     $line_width <= $width ) {
                ###########################################
                if ( sprintf("%.2f", $text_object->advancewidth( $paragraph[0] ) + $line_width) <= sprintf("%.2f", $width) ) {
                    push(@line, shift(@paragraph));
                    next if @paragraph;
                } else {
                    # this should never happen, but just in case, to
                    # prevent an infinite loop...
                    warn "!!! Error !!! ".sprintf("%.2f", $text_object->advancewidth( $paragraph[0] ) + $line_width)." <= ".sprintf("%.2f", $width); # KZ debug
