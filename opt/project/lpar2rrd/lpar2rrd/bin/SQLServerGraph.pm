...
my $main_data_dir = "$inputdir/data/SQLServer";
...
sub signpost {
  ...
  elsif ( $item =~ m/^sqlserver/ ) {
    return graph_default_nolegend( $acl_check, $host, $server, $lpar, $item, $colors );
  }
  else {
    return 0;
  }
...
sub graph_default {
  ...
  if ( $item =~ m/_a_/ ) {
    $color = "#";
    $color .= SQLServerDataWrapper::basename( $item, '_a_' );
    $item = substr( $item, 0, index( $item, '_a_' ) );
  }
  my $pages_ref = SQLServerDataWrapper::get_pages($type);

  my $page     = SQLServerDataWrapper::basename( $item, '__' );
  my $type_new = SQLServerDataWrapper::get_type_from_page($page);
  my $rrd      = SQLServerDataWrapper::get_filepath_rrd( { type => $type_new, uuid => $server, id => $host, acl_check => $acl_check } );
  ...
  my $rrd_name   = $pages_ref->{$type}->{$page}->{$dunno};
...
sub graph_default_nolegend {
  ...
  my $type_new = SQLServerDataWrapper::get_type_from_page($_page);
  $rrd = SQLServerDataWrapper::get_filepath_rrd( { type => $type_new, uuid => $server, id => $host, acl_check => $acl_check } );
  my $legend = SQLServerDataWrapper::graph_legend($_page);
  my @values = keys %{ $pages{$type}{$_page} };
  ...
  foreach my $val (@values) {
    my $rrdval = $pages{$type}{$_page}{$val};
    $cmd_def  .= " DEF:preview-$cur_hos-$rrdval=\"$rrd\":$rrdval:AVERAGE";
    $cmd_cdef .= " CDEF:view-$cur_hos-$rrdval=preview-$cur_hos-$rrdval,$legend->{denom},/";
    ...
    $cmd_legend .= " GPRINT:view-$cur_hos-$rrdval:AVERAGE:\" %6." . "$decimals" . "lf\"";
    ...
    $cmd_legend .= " PRINT:view-$cur_hos-$rrdval:MAX:\" %6." . "$decimals" . "lf $del asd $del $label $del cur_hos\"";
    $cmd_legend .= " COMMENT:\\n";
    $cur_hos++;
  }
