...
if ( defined $ENV{'CONTENT_TYPE'} && $ENV{'CONTENT_TYPE'} =~ "multipart/form-data" ) {
  ...
  if ($file) {
    ...
    # if (length($file) <= 1000000) {
    if (0) {
      &result( 0, "File is too small, are you sure it's the upgrade tarball??" );
    }
    else {
      my $tmpfilename = $cgi->tmpFileName($file);

      open( UPLOADFILE, ">/tmp/$filename" ) or die "$!";
      binmode UPLOADFILE;
      while (<$file>) {
        print UPLOADFILE;
      }
      ...
      chmod 0664, "/tmp/$filename";
      my $udir = ( split /\.tar/, $filename )[0];
      my $txt  = `cd /tmp; tar xf /tmp/$filename`;
...
if ( $PAR{cmd} eq "form" ) {    ### Generate upload form
  ...
  if ( $ENV{VM_IMAGE} ) {
    print <<_MARKER_;
    <p>Download upgrade / patch package (*.tar) from <a href='https://www.lpar2rrd.com/download.htm' target="blank"><b>LPAR2RRD</b></a> to your computer and then upload it via following form:<br>
    <form id="upgrade-form" action="/lpar2rrd-cgi/upgrade.sh" method="post" enctype="multipart/form-data">
...
_MARKER_
    ...
    print "<p>This feature is only active when running as VMware appliance. <a href='https://www.lpar2rrd.com/LPAR2RRD_and_STOR2RRD_Virtual_Appliance.htm'>More info...</a></p>";
  }
}
