...
foreach my $file ( sort @files ) {
  ...
  foreach my $podKey ( keys %{ $data->{pod} } ) {
    ...
    print "\n - $podKey";
    $rrd_filepath = KubernetesDataWrapper::get_filepath_rrd( { type => 'pod', uuid => $labels->{name_to_uuid}->{$podKey} } );
    ...
    foreach my $networkKey ( sort keys %{ $data->{pod}->{$podKey}->{network} } ) {
      print "$networkKey ";
      $rrd_filepath = KubernetesDataWrapper::get_filepath_rrd( { type => 'network', parent => $labels->{name_to_uuid}->{$podKey}, uuid => $networkKey } );
    ...
    foreach my $containerKey ( sort keys %{ $data->{pod}->{$podKey}->{container} } ) {
      ...
      $rrd_filepath = KubernetesDataWrapper::get_filepath_rrd( { type => 'container', uuid => $new_name } );
      my $created_rrd = ( -f $rrd_filepath ) ? 1 : 0;
  ...
  foreach my $nodeKey ( keys %{ $data->{node} } ) {
    ...
    $rrd_filepath = KubernetesDataWrapper::get_filepath_rrd( { type => 'node', uuid => $labels->{name_to_uuid}->{$nodeKey} } );
    unless ( -f $rrd_filepath ) {
...
foreach my $cluster (@clusters) {
  ...
  my @nodes = @{ KubernetesDataWrapper::get_items( { item_type => 'node', parent_type => 'cluster', parent_id => $cluster_id } ) };
  print "\nNodes: ";
  foreach my $node (@nodes) {
    my ( $node_uuid, $node_label ) = each %{$node};
    print "$node_label ";
  }
  my @pods = @{ KubernetesDataWrapper::get_items( { item_type => 'pod', parent_type => 'cluster', parent_id => $cluster_id } ) };
  print "\nPods: ";
  foreach my $pod (@pods) {
