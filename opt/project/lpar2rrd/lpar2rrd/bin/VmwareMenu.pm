# VmwareMenu.pm
# page types and associated tools for generating front-end menu and tabs for Vmware
...
if ( defined $ENV{XORMON} && $ENV{XORMON} ) {
  my $file = "$basedir/etc/links_vmware.json";
...
sub url_new_to_old {
  ...
  if ( $page_type eq "Heatmap" ) {
    return {
      url_base => 'heatmap-vmware.html',
      params   => {}
    };
  }
  elsif ( $page_type eq "resource_configuration_advisor" ) {
    return {
      url_base => 'gui-cpu_max_check_vm.html',
      params   => {}
    };
  }
  elsif ( $page_type eq "serversvm" ) {
    return {
      url_base => '',
      params   => {
        lpar => '',
  ...
  elsif ( $page_type eq "dstr-table-top" ) {
    return {
      url_base => '',
      params   => {
        lpar => '',
    ...
    return {
      url_base => '/lpar2rrd-cgi/histrep.sh?mode=globalvm',
      params   => {}
    };
  }
