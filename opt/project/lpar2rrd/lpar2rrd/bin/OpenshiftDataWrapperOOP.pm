# OpenshiftDataWrapperOOP.pm
# interface for accessing Openshift data:

package OpenshiftDataWrapperOOP;
...
sub get_items {
  ...
  if ( $params{item_type} eq 'node' ) {
    ...
    if ( defined $params{parent_type} && defined $params{parent_id} ) {
      ...
      elsif ( $params{parent_type} eq 'namespace' || $params{parent_type} eq 'project' ) {
  ...
  elsif ( $params{item_type} eq 'namespace' || $params{item_type} eq 'project' ) {
    if ( defined $params{parent_type} && defined $params{parent_id} ) {
      if ( $params{parent_type} eq 'cluster' ) {
        my $cluster             = $params{parent_id};
...
sub get_filepath_rrd {
  ...
  # params: { type => '(pod|node|container|network)', uuid => 'DEADBEEF' }
...
sub get_all_containers {
  ...
  foreach my $pod ( keys %{$pods} ) {
    foreach my $container ( @{ $pods->{$pod} } ) {
      ...
      #if ( defined $self->get_filepath_rrd({type => 'container', $uuid => $container}) && -f $self->get_filepath_rrd({type => 'container', $uuid => $container}) ) {
...
sub get_conf_section {
  ...
  if ( $section eq 'labels' ) {
    ...
    return $self->{project}{infrastructure};
  }
...
sub is_granted {
  ...
  if ( $self->{aclx} ) {
    return $self->{aclx}->isGranted( { hw_type => 'OPENSHIFT', item_id => $uuid, match => 'granted' } );
  }
