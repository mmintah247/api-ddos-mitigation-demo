...
my $result_file      = "lpar2rrd-virtual-result-$$.txt";
...
#`echo "01 $QUERY_STRING" >> /tmp/xx66`;

#`echo "$QUERY_STRING" >> /tmp/ee8`;

( my $week, my $month, my $year, my $util, my $time_range, my $inp_data, my $hmc, my @server_list ) = split( /&/, $QUERY_STRING );
...
if ( $time_range =~ m/week/ ) {
  ...
  print "<tr><td><center><h3>Virtual CPU for $month/$year</center></h3></td></tr>";
  print "<tr><td><img src=\"/lpar2rrd-cgi/virtual-work.sh?util=$util&input=$inp_data&week=$week&month=$month&year=$year&result=$result_file&weekno=0&print=1&xport=0&smonth=0&emonth=0&hmc=$hmc&@server_list\"></td>";
}
else {
  # one month graph(s)
  ...
  for ( my $week_no = 1; $week_no < 7; $week_no++ ) {
    ...
    if ( $stime > 0 ) {
      ...
      #print "<tr><td><img src=\"/lpar2rrd-cgi/virtual-work.sh?util=$util&input=$inp_data&week=$week&month=$month&year=$year&result=$result_file&weekno=$week_no&print=0&hmc=$hmc&@server_list\"></td>";
      print "<img src=\"/lpar2rrd-cgi/virtual-work.sh?util=$util&input=$inp_data&week=$week&month=$month&year=$year&result=$result_file&weekno=$week_no&print=0&xport=0&smonth=0&emonth=0&hmc=$hmc&@server_list\" style=\"visibility:hidden\">\n";
      $keep_html[$keep_html_indx] = "/lpar2rrd-cgi/virtual-work.sh?util=$util&input=$inp_data&week=$week&month=$month&year=$year&result=$result_file&weekno=$week_no&print=1&xport=1&smonth=0&emonth=0&hmc=$hmc&@server_list";
      $keep_html_indx++;
      $num_of_weeks++;
    }
  }
  ...
  $keep_html[$keep_html_indx] = "/lpar2rrd-cgi/virtual-work.sh?util=$util&input=$inp_data&week=$week&month=$month&year=$year&result=$result_file&weekno=0&print=1&xport=1&smonth=$month_sum_stime&emonth=$month_sum_etime&hmc=$hmc&@server_list";
  ...
  for ( my $week_no = 1; $week_no < 7; $week_no++ ) {
    ...
    if ( $stime > 0 ) {
      ...
      my $etime = $stime + 604800;
      $week = $stime . " " . $etime;
      print "<tr><td><img src=\"/lpar2rrd-cgi/virtual-work.sh?util=$util&input=$inp_data&week=$week&month=$month&year=$year&result=$result_file&weekno=$week_no&print=1&xport=0&smonth=0&emonth=0&hmc=$hmc&@server_list\"></td>";
      $num_of_weeks++;
    }
  }
}
...
sub print_summary {
  ...
  if ( $indx == $MAX_WAIT_TIME ) {
    ...
    error("virtual-cpu-acc-cgi.pl: $result_file_full does not exist or timed out ($MAX_WAIT_TIME) or too less row : $num_of_weeks : $num_of_weeks_act : $result_all_text");
    err_html("virtual-cpu-acc-cgi.pl: $result_file_full does not exist or timed out ($MAX_WAIT_TIME) or too less row : $num_of_weeks : $num_of_weeks_act : $result_all");
  }
  else {
    if ( !-f $result_file_full ) {
      error("virtual-cpu-acc-cgi.pl: $result_file_full does not exist");
      err_html("virtual-cpu-acc-cgi.pl: $result_file_full does not exist");
      return 1;
    }
    ...
    if ( $util == 0 ) {
      ...
      print "\n<center><table border=\"1\"><tr><th>week no</th><th>Days in a week</th><th>Avrg CPU utilization</th><th>CSV</th></tr>";
    }
    my $week_tot       = 0;
    my $days_tot       = 0;
    my $keep_html_indx = 0;
