...
my $main_data_dir = "$inputdir/data/PostgreSQL";
...
sub graph_default {
  ...
  if ( $item =~ m/_a_/ ) {
    $color = "#";
    $color .= PostgresDataWrapper::basename( $item, '_a_' );
    $item = substr( $item, 0, index( $item, '_a_' ) );
  }
  my $pages_ref = PostgresDataWrapper::get_pages($type);

  my $page     = PostgresDataWrapper::basename( $item, '__' );
  my $type_new = PostgresDataWrapper::get_type_from_page($page);
  my $rrd      = PostgresDataWrapper::get_filepath_rrd( { type => $type_new, uuid => $server, id => $host, acl_check => $acl_check } );
...
sub graph_views {
  ...
  my $type_new = PostgresDataWrapper::get_type_from_page($_page);
  $rrd = PostgresDataWrapper::get_filepath_rrd( { type => $type_new, uuid => $server, id => $host, acl_check => $acl_check } );
